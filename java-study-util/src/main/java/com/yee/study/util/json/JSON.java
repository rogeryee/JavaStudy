...
public class JSON {
     ...
     * 默认下划线命名策略实现
     */
    private static JSON DEFAULT_WITH_SNAKE_CASE_NAME = build(DEFAULT_DATE_PATTERN, new PropertyNamingStrategy.SnakeCaseStrategy(), false);

    /**
     * 默认全下划线命令策略实现
     */
    private static JSON DEFAULT_WITH_FULL_SNAKE_CASE_NAME = build(DEFAULT_DATE_PATTERN, new PropertyNamingStrategy.SnakeCaseStrategy(), true);
     ...
     * @return
     */
    public <T> T parseToObject(byte[] jsonData, int offset, int length, final Type type) {
        try {
            return jacksonMapper.readValue(jsonData, offset, length, new TypeReference<T>() {
                public Type getType() {
                    return type;
                }
            });
