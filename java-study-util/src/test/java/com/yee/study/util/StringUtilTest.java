...
import static com.yee.study.util.StringUtil.*;
...
public class StringUtilTest
{
    ...
    public void testSplit2Collection()
    {
        ...
        Assert.assertEquals("d", list.get(6));
    ...
    {
        final String xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<root>" + "<head><name>head</name><value>head</value></head>" + "<body><name>body</name><value>body</value></body>" + "</root>";
    ...
    {
        final String xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<root>" + "<head><ext>head</ext></head>" + "<body><ext>123</ext></body>" + "</root>";
    ...
    {
        String xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<root>" + "<head><ext>head</ext></head>" + "<body><ext>123</ext></body>" + "</root>";
        String expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<root>" + "<body><ext>123</ext></body>" + "</root>";
        String actual = replaceXML(xml, "head", new String[]{""});
        Assert.assertEquals(expected, actual);

        expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<root>" + "<head>head</head>" + "<body>body</body>" + "</root>";
        actual = replaceXML(xml, "ext", new String[]{"head", "body"});
        Assert.assertEquals(expected, actual);

        expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<root>" + "<head>abc</head>" + "<body>abc</body>" + "</root>";
    ...
    {
        String xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<root>" + "<head><ext>head</ext></head>" + "<body><ext></ext></body>" + "</root>";
        String expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<root>" + "<head></head>" + "<body><ext></ext></body>" + "</root>";
        String actual = replaceXMLBetween(xml, "head", new String[]{""});
        Assert.assertEquals(expected, actual);

        expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<root>" + "<head><ext>head</ext></head>" + "<body><ext>body</ext></body>" + "</root>";
        actual = replaceXMLBetween(xml, "ext", new String[]{"head", "body"});
        Assert.assertEquals(expected, actual);

        expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<root>" + "<head><ext>abc</ext></head>" + "<body><ext>abc</ext></body>" + "</root>";
    ...
    public void testJoin()
    {
        ...
        final Collection<?> cols = ArrayUtil.asList(0.1, ArrayUtil.asList(new BigDecimal("1"), 'a', "你", "", " ", null, '\n'), ArrayUtil.asList("a", "2"));
    ...
    public void testLengthAsByte()
    {
        ...
        Assert.assertEquals(Charsets.UTF_8.equals(Charsets.defaultCharset()) ? 13 : 11, lengthAsByte("123abc 测试", null));
